...
void buttonTask(void* param)
{
    ...
    button_data[BTN_MODE1].bitmask = BTN_MODE1_BITMASK;
    button_data[BTN_MODE1].count = 0;

    button_data[BTN_MODE2].gpio = GPIO_BTN_MODE2;
    button_data[BTN_MODE2].bitmask = BTN_MODE2_BITMASK;
...
bool getBtnState(uint8_t button)
{
    ...
    if (xEventGroupGetBits(buttoneventgroup) & button_data[button].bitmask)
    {
        xEventGroupClearBits(buttoneventgroup, button_data[button].bitmask);
        return true;       
    }
